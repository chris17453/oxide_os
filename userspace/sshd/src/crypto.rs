...
pub fn init_host_key() -> TransportResult<()> {
    unsafe {
        ...
        let fd = open("/etc/ssh/ssh_host_ed25519_key", (O_WRONLY | O_CREAT | O_TRUNC) as u32, 0o600);
        if fd >= 0 {
...
impl SshCipher {
    ...
    pub fn encrypt_packet(&mut self, payload: &[u8], seq: u32) -> TransportResult<Vec<u8>> {
        ...
        let (ciphertext, tag) = chacha20_poly1305_encrypt(&plaintext, &self.key, &nonce, &encrypted_len);
    ...
    pub fn decrypt_packet(&mut self, fd: i32, seq: u32) -> TransportResult<Vec<u8>> {
        ...
        // Decrypt and verify
        let plaintext = chacha20_poly1305_decrypt(&ciphertext, &self.key, &nonce, &encrypted_len, &tag)?;
