...
use efflux_input::{InputDeviceInfo, InputDeviceType, InputEvent, EventType, KeyValue};
use spin::Mutex;
...
impl VirtioInput {
    ...
    pub fn init(&mut self) -> Result<(), &'static str> {
        ...
        self.device_id = Some(efflux_input::register_device_info(info));
        ...
        if let Some(device_id) = self.device_id {
            match event.event_type {
                ...
                1 => {
                    ...
                    efflux_input::report_key(device_id, event.code, value);
                }
                2 => {
                    // EV_REL
                    efflux_input::report_rel(device_id, event.code, event.value as i32);
                }
                3 => {
                    // EV_ABS
                    efflux_input::report_abs(device_id, event.code, event.value as i32);
                }
                _ => {}
            }
        }
