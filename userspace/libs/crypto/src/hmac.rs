...
//!
//! RFC 2104 compliant. Forked from kernel crypto, now runs in the gutter
//! of userspace where the real TLS handshakes happen.
//! — ColdCipher: "Two hashes, one key. The inner sanctum of message authentication."

use crate::sha256::{sha256, Sha256};
use crate::sha384::{sha384, Sha384};
use crate::sha512::{sha512, Sha512};

/// HMAC-SHA-512
/// — ColdCipher: "The 128-byte block size means your key gets the VIP treatment."
...
/// HMAC-SHA-384
/// — ColdCipher: "Same 128-byte blocks as SHA-512, but we only keep 48 bytes of the output."
...
impl HmacSha384 {
    /// Block size for SHA-384 (same as SHA-512)
    const BLOCK_SIZE: usize = 128;

    /// Create new HMAC-SHA-384 with key
    pub fn new(key: &[u8]) -> Self {
        let mut key_block = [0u8; Self::BLOCK_SIZE];
    ...
    /// Finalize and return the 48-byte MAC
...
/// HMAC-SHA-256
/// — ColdCipher: "64-byte blocks. Compact, efficient, the workhorse of TLS 1.3."
...
/// Compute HMAC-SHA-384 of data with key
...
/// — ColdCipher: "Timing attacks die here. Every byte gets its turn, no early exits."
...
mod tests {
    ...
    fn test_hmac_sha384_rfc4231_test1() {
        // Test Case 1 from RFC 4231 — HMAC-SHA-384
        let key = [0x0bu8; 20];
        let data = b"Hi There";
        let mac = hmac_sha384(&key, data);
        // Expected starts with: afd03944d84895626b0825f4ab46907f
        assert_eq!(mac[0], 0xaf);
        assert_eq!(mac[1], 0xd0);
        assert_eq!(mac[2], 0x39);
    }
