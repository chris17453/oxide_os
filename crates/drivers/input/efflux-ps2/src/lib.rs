...
impl Ps2Keyboard {
    ...
    pub fn handle_scancode(&self, scancode: u8) {
        ...
        if let Some((keycode, pressed)) = keymap.process_scancode(scancode) {
            ...
            match keycode {
                efflux_input::KEY_LEFTSHIFT | efflux_input::KEY_RIGHTSHIFT => {
                    self.shift.store(pressed, Ordering::SeqCst);
                }
                efflux_input::KEY_LEFTCTRL | efflux_input::KEY_RIGHTCTRL => {
                    self.ctrl.store(pressed, Ordering::SeqCst);
                }
                efflux_input::KEY_LEFTALT | efflux_input::KEY_RIGHTALT => {
            ...
            efflux_input::report_key(self.device_id() as usize, keycode, value);
            efflux_input::report_sync(self.device_id() as usize);
        }
    }
...
impl Ps2Mouse {
    ...
    fn process_packet(&self, packet: &[u8; 4]) {
        ...
        if (old_buttons ^ new_buttons) & 0x01 != 0 {
            ...
            efflux_input::report_key(device_id, BTN_LEFT, value);
        }

        // Right button
        if (old_buttons ^ new_buttons) & 0x02 != 0 {
            let value = if new_buttons & 0x02 != 0 { KeyValue::Pressed } else { KeyValue::Released };
            efflux_input::report_key(device_id, BTN_RIGHT, value);
        }

        // Middle button
        if (old_buttons ^ new_buttons) & 0x04 != 0 {
            let value = if new_buttons & 0x04 != 0 { KeyValue::Pressed } else { KeyValue::Released };
            efflux_input::report_key(device_id, BTN_MIDDLE, value);
        }

        // Movement
        if dx != 0 {
            efflux_input::report_rel(device_id, REL_X, dx as i32);
        }
        if dy != 0 {
            // PS/2 mouse Y axis is inverted
            efflux_input::report_rel(device_id, REL_Y, -(dy as i32));
        }
        ...
        if self.has_wheel.load(Ordering::SeqCst) {
            ...
            if dz != 0 {
                efflux_input::report_rel(device_id, REL_WHEEL, -(dz as i32));
            }
        }

        efflux_input::report_sync(device_id);
    }
}
...
pub fn init() -> bool {
    ...
    if keyboard.init() {
        let id = efflux_input::register_device(keyboard.clone());
    ...
    if mouse.init() {
        let id = efflux_input::register_device(mouse.clone());
