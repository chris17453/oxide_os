...
//!
//! RFC 8439 compliant. The stream cipher that doesn't need lookup tables,
//! making it immune to cache-timing attacks. Poly1305 seals the deal.
//! — ColdCipher: "ARX construction — add, rotate, XOR. Simple ops, unbreakable cipher."
...
/// — ColdCipher: "No S-boxes, no GF multiplication. Just ARX and vibes."
...
impl ChaCha20Poly1305 {
    ...
    pub fn encrypt(&self, nonce: &ChaChaNonce, plaintext: &[u8], aad: &[u8]) -> Vec<u8> {
        // Generate Poly1305 key from first 32 bytes of keystream (counter=0)
    ...
    pub fn decrypt(
        ...
        // Verify tag before decrypting — authenticate first, always
    ...
    fn chacha20_block(&self, nonce: &ChaChaNonce, counter: u32) -> [u8; 64] {
        ...
        // Constants "expand 32-byte k" — the ChaCha20 nothing-up-my-sleeve constant
        state[0] = 0x61707865;
        state[1] = 0x3320646e;
        state[2] = 0x79622d32;
        state[3] = 0x6b206574;
        ...
        // 20 rounds (10 double rounds) — the ChaCha20 diffusion core
        for _ in 0..10 {
            // Column rounds
    ...
    /// ChaCha20 stream cipher — XOR data with keystream
...
/// — ColdCipher: "One key, one message. Reuse the key and all bets are off."
