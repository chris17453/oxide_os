...
//! No kernel dependencies - response delivery via callback.
//!
//! -- GraveShift: Sequence handler - translates CSI/ESC/SGR into terminal mutations
...
pub enum MouseMode {
    /// No mouse tracking
    None,
    /// X10 compatibility mode (CSI ?9 h)
    X10,
    /// Normal tracking mode (CSI ?1000 h)
    Normal,
...
pub enum MouseEncoding {
    /// X10 encoding (default)
    X10,
    /// UTF-8 encoding (CSI ?1005 h)
    Utf8,
    /// SGR encoding (CSI ?1006 h)
    Sgr,
    /// Urxvt encoding (CSI ?1015 h)
    Urxvt,
}
...
pub enum Charset {
    /// ASCII
    Ascii,
    /// DEC Special Graphics - single-line box drawing
    DecSpecialGraphics,
...
/// Callback type for sending terminal query responses (DSR, DA, etc.)
...
impl Handler {
    ...
    /// Send a response via the callback (if registered)
