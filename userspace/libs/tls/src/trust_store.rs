//! Root CA Trust Store — embedded DER certificates.
//!
//! Like Linux: trust anchors compiled into the binary. Future: load from
//! `/etc/ssl/certs/ca-bundle.crt`. For now, we embed the roots that sign
//! ~80% of the internet's HTTPS traffic.
//!
//! — VeilAudit: "Three root CAs. Three pillars of trust for the entire internet.
//!   If any of these gets compromised, we all have bigger problems than our
//!   little OS. But at least we'll reject the forgeries."

extern crate alloc;
use alloc::vec::Vec;
use crate::x509::Certificate;

/// ISRG Root X1 — Let's Encrypt's root CA.
/// Signs ~30% of the internet. 4096-bit RSA, valid until 2035-06-04.
/// — VeilAudit: "The nonprofit that made HTTPS free. Their root is sacred."
static ISRG_ROOT_X1_DER: &[u8] = &[
...
/// DigiCert Global Root G2 — signs Google, Facebook, many CDNs.
/// 2048-bit RSA, valid until 2038-01-15.
/// — VeilAudit: "DigiCert: the CA that corporations trust with their wallets.
///   Root G2 is their current workhorse."
static DIGICERT_GLOBAL_ROOT_G2_DER: &[u8] = &[
...
/// Signs all *.google.com, *.youtube.com, *.gstatic.com.
/// 4096-bit RSA, valid until 2036-06-22.
/// — VeilAudit: "Google's root. If you want to talk to Google over HTTPS,
///   you must trust this certificate. That's how PKI works."
static GTS_ROOT_R1_DER: &[u8] = &[
...
/// AAA Certificate Services — Comodo/Sectigo root CA.
/// 2048-bit RSA, valid until 2028-12-31. Cross-signs SSL.com intermediates
/// used by Cloudflare's current certificate chain for example.com and others.
/// — VeilAudit: "The root behind Cloudflare's ECC chain. Without this,
///   half the CDN-hosted internet is untrusted. Added after three hours of
///   'chain signature invalid' debugging."
static AAA_CERTIFICATE_SERVICES_DER: &[u8] = &[
