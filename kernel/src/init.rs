    ...
    let net_initialized = if let Some(device) = net_devices.first() {
        ...
        match dhcp_result {
            Ok(lease) => {
                ...
                // — GraveShift: Persist DHCP DNS servers to /etc/resolv.conf.
                // Without this, userspace resolvers (libc::dns, nslookup, ping) default
                // to 8.8.8.8 which is unreachable from QEMU's user-mode NAT. The DHCP
                // forwarder at 10.0.2.3 is the only working DNS path. — GraveShift
                if !lease.dns_servers.is_empty() {
                    use alloc::string::String;
                    let mut resolv_content = String::from("# Generated by OXIDE networkd\n# Do not edit manually\n");
                    for dns in &lease.dns_servers {
                        resolv_content.push_str("nameserver ");
                        resolv_content.push_str(&alloc::format!("{}", dns));
                        resolv_content.push('\n');
                    }
                    // — GraveShift: Write via VFS. /etc should exist from initramfs.
                    if let Ok(etc_dir) = vfs::mount::GLOBAL_VFS.lookup("/etc") {
                        let vnode_result = etc_dir.lookup("resolv.conf")
                            .or_else(|_| etc_dir.create("resolv.conf", vfs::Mode::new(0o644)));
                        match vnode_result {
                            Ok(vnode) => {
                                let _ = vnode.truncate(0);
                                let _ = vnode.write(0, resolv_content.as_bytes());
                                let _ = writeln!(writer, "[NET] Wrote /etc/resolv.conf ({} servers)", lease.dns_servers.len());
                            }
                            Err(e) => {
                                let _ = writeln!(writer, "[NET] Failed to open/create resolv.conf: {:?}", e);
                            }
                        }
                    } else {
                        let _ = writeln!(writer, "[NET] /etc not found, skipping resolv.conf");
                    }
                }
            }
            Err(e) => {
