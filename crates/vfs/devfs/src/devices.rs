...
/// Push a character to the console through the line discipline
///
/// Feeds input through the TTY line discipline which handles:
/// - Character echo (if ECHO enabled)
/// - Line editing (backspace, etc. if ICANON enabled)
/// - Signal generation (Ctrl+C, Ctrl+\, Ctrl+Z)
/// - Cooked vs raw mode processing
///
/// If a task is blocked waiting for input, it will be woken up.
pub fn console_push_char(ch: u8) {
    let mut ldisc = get_console_ldisc();

    // Feed character through line discipline
    if let Some(ldisc_inner) = ldisc.as_mut() {
        // Echo callback: write echo data back to console
        let signal = ldisc_inner.input_char(ch, |echo_data| {
            // Write echoed data back to terminal/serial
            unsafe {
        ...
        // Handle signals generated by line discipline
...
impl VnodeOps for ConsoleDevice {
    ...
    fn read(&self, _offset: u64, buf: &mut [u8]) -> VfsResult<usize> {
        ...
        // Block until at least one character is available from line discipline
        loop {
            ...
            // Try to read from line discipline
            {
                let mut ldisc = get_console_ldisc();
                if let Some(ldisc_inner) = ldisc.as_mut() {
            ...
            // we blocked. Re-check the line discipline to avoid sleeping forever.
            {
                let ldisc = get_console_ldisc();
                if let Some(ldisc_inner) = ldisc.as_ref() {
