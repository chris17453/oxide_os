...
impl Handshake {
    ...
    pub fn process_server_hello(&mut self, data: &[u8]) -> Result<(), &'static str> {
        ...
        // — ColdCipher: Verify X25519 against RFC 7748 test vector
        {
            ...
            let alice = X25519SecretKey::generate(&alice_priv_bytes);
            let bob_pub = X25519PublicKey::from_bytes(&bob_pub_bytes).map_err(|_| "X25519 test: bad pubkey")?;
